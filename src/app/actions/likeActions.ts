...
export async function toggleLikeMember(targetUserId: string, isLiked: boolean) {
  try {
    ...
    console.log("Toggle like member error: ", error);
    throw error;
  }
}
...
export async function fetchLikedMembers(type = "source") {
  try {
    ...
    console.log("fetchLikedMembers ;", error);
    throw error;
  }
}
...
async function fetchMutualLikes(userId: string) {
  const likedUsersIds = await prisma.like.findMany({
    where: {
      sourceUserId: userId,
    },
  ...
  const likedIds = likedUsersIds.map((x) => x.targetUserId);
